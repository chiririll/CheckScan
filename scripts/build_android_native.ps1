...
$goMod = Join-Path $root "services\providers"
$jniRoot = Join-Path $root "packages\providers_native\android\src\main\jniLibs"
...
function Get-NdkRoot {
    ...
    if (Test-Path $localProps) {
        foreach ($line in Get-Content $localProps) {
            if ($line -match '^\s*ndk\.dir=(.+)$') {
                $path = $Matches[1].Trim().Replace("\\", "\")
                if (Test-Path $path) { return $path }
            }
            if ($line -match '^\s*sdk\.dir=(.+)$') {
                $sdkCandidates += $Matches[1].Trim().Replace("\\", "\")
            }
        }
    }
    ...
    throw "NDK not found. Set ANDROID_NDK_HOME or ndk.dir in android/local.properties."
}
...
function Get-ProverkaToken {
    ...
    if (Test-Path $localProps) {
        foreach ($line in Get-Content $localProps) {
            if ($line -match '^\s*proverkacheka\.token=(.+)$') {
